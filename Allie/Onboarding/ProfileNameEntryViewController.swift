...
class ProfileNameEntryViewController: BaseViewController, UITextViewDelegate, UIGestureRecognizerDelegate {
	var sendDataAction: ((OCKBiologicalSex, String, [String]) -> Void)?
	...
	func setButtons() {
		let labelText = NSLocalizedString("BIOLOGICAL_SEX", comment: "Biological Sex")
		sexLabel.attributedText = labelText.with(style: .regular20, andColor: .grey, andLetterSpacing: -0.408)
	...
	@IBAction func nextBtnTapped(_ sender: Any) {
		...
		sendDataAction?(selectedGender, lastName, givenNames)
	}
}
