...
class CarePlanCell: UICollectionViewCell {
	...
	func configureCell(for task: CHTasks) {
		title.attributedText = task.first?.category?.capitalized.attributedString(style: .bold20, foregroundColor: .allieBlack, letterSpacing: -0.41)
		task.forEach { chTask in
			...
			taskTitle.attributedText = chTask.title?.attributedString(style: .medium17, foregroundColor: .allieBlack, letterSpacing: 0.02)
