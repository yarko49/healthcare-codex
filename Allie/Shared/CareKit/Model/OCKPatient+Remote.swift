...
public extension AlliePatientExtensible {
	...
	var weight: Int? {
		get {
			guard let intString = userInfo?["weight"] else {
				return nil
			}
		...
		set {
			...
			setUserInfo(item: value, forKey: "weight")
		}
	}

	var height: Int? {
		get {
			guard let intString = userInfo?["height"] else {
				return nil
			}
			return Int(intString)
		}
		set {
			let value = String(newValue ?? 0)
			setUserInfo(item: value, forKey: "height")
		}
	}
