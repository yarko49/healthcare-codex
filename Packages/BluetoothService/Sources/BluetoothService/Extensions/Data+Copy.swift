...
internal extension Data {
	...
	func subdataNoCopy(in range: Range<Int>) -> Data {
		...
		if count > Data.inlineBufferSize {
			return withUnsafeBytes { (buffer: UnsafeRawBufferPointer) in
				Data(bytesNoCopy: UnsafeMutableRawPointer(mutating: buffer.baseAddress!.advanced(by: range.lowerBound)), count: range.count, deallocator: .none)
			}
		} else {
	...
	func withUnsafeBytes<Result>(in range: Range<Int>, _ block: (UnsafeRawBufferPointer) throws -> Result) rethrows -> Result {
		try withUnsafeBytes {
			try block(UnsafeRawBufferPointer(rebasing: $0[range]))
		}
	}

	@usableFromInline
	func suffixNoCopy(from index: Int) -> Data {
		subdataNoCopy(in: index ..< count)
	}
...
internal extension Data {
	...
	static var inlineBufferSize: Int {
		...
		typealias Buffer = (UInt8, UInt8, UInt8, UInt8, UInt8, UInt8) // len  //enum
